...
//brand,proname,label,amount,store_status,order_id,order_status,
    ...
    public List<ListModel> getFubDatail(String order_id) {
        ...
        final List<ListModel> fulist = new ArrayList<ListModel>();
        try {
            jdbcTemplate.query("select merchant_name,merchant_tel,merchant_address,freight_type,freight_number,reach_time, order_status, man_telephone,order_time,update_time,production_predict_time,comment from fu_order where id =?", new Object[]{order_id},
                    new RowCallbackHandler() {
                        @Override
                        public void processRow(ResultSet rs) throws SQLException {
                            ListModel lm = new ListModel();
                            ...
                            lm.setComment(rs.getString("comment"));
                            fulist.add(lm);
                        }
                    });
            return fulist;
    ...
    public List<ListModel> GetFuListFinished(String order_status, String man_telephone) {
        ...
        if (user.getType().equals("业务人员")) {
            try {
                jdbcTemplate.query("select id, merchant_name,merchant_tel,merchant_address,freight_type,freight_number,reach_time, order_status, man_telephone,order_time,update_time,production_predict_time,comment from fu_order where order_status =? and man_telephone=? limit 100", new Object[]{order_status, man_telephone},
                        new RowCallbackHandler() {
                            @Override
                            public void processRow(ResultSet rs) throws SQLException {
                                ListModel lm = new ListModel();
            ...
            try {
                jdbcTemplate.query("select id, merchant_name,merchant_tel,merchant_address,freight_type,freight_number,reach_time, order_status, man_telephone,order_time,update_time,production_predict_time,comment from fu_order where order_status =? limit 100", new Object[]{order_status},
                        new RowCallbackHandler() {
                            @Override
                            public void processRow(ResultSet rs) throws SQLException {
                                ListModel lm = new ListModel();
